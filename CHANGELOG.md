...
* **wrap:** `headroom wrap claude --1m` preserves the 1M context window. Behind a custom `ANTHROPIC_BASE_URL` (the proxy) Claude Code drops the `context-1m` beta header and caps the window at 200k for entitled subscription users; the opt-in flag appends the `[1m]` suffix to your selected model (via `--model` or `ANTHROPIC_MODEL`) on the launched process so the 1M window activates through Headroom. The flag requires a model to already be known — it does not default to Opus or any other model when none is set, since most users select a model via `--model`/the in-app picker rather than `ANTHROPIC_MODEL`, and guessing would silently override that choice. Launch-time only: switching models afterward via Claude Code's own `/model` picker goes through the same upstream bug and drops back to 200k (anthropics/claude-code#68522) ([#1158](https://github.com/chopratejas/headroom/issues/1158)).
