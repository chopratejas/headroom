...
* **proxy:** fix token-mode `/v1/chat/completions` producing zero compression (`tokens_saved=0`, `transforms_applied=[]`) on tool-light/prose conversations while `/v1/messages` on the same proxy compressed ([#987](https://github.com/chopratejas/headroom/issues/987)). The OpenAI handler overwrote the session prefix-tracker frozen count with `compute_frozen_count(messages)` directly; that heuristic returns `len(messages) - 1` when every leading message is "stable", freezing the entire live zone. Now clamped with `min(prefix_tracker, compute_frozen_count)`, mirroring the Anthropic handler so the heuristic can only lower the boundary, never extend past the provider-confirmed cached prefix.
