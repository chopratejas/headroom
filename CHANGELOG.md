...
* **codex:** respect `CODEX_HOME` when `headroom wrap codex` writes provider, MCP, memory, backup, and global `AGENTS.md` config, and fail fast with a hint when `unwrap codex` cannot see a custom Codex home because `CODEX_HOME` is unset.
