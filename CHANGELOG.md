...
* **wrap/claude (vertex):** `headroom wrap claude` in Vertex mode now forwards the user's custom `ANTHROPIC_VERTEX_BASE_URL` to the proxy as `VERTEX_TARGET_API_URL`. Wrap overwrites `ANTHROPIC_VERTEX_BASE_URL` with the proxy URL but never passed the original on, so the proxy fell back to the default Google Vertex hosts and requests to a custom Vertex gateway 404'd. The original endpoint is captured before the overwrite and threaded through `_ensure_proxy`/`_start_proxy` to the proxy ([#1476](https://github.com/headroomlabs-ai/headroom/issues/1476)).
