...
* **proxy:** allow direct OpenAI chat-completions forwarding to use a custom upstream path via `--openai-chat-path` / `HEADROOM_OPENAI_CHAT_PATH`, while keeping Headroom's public route at `/v1/chat/completions` ([#1203](https://github.com/headroomlabs-ai/headroom/issues/1203)).
