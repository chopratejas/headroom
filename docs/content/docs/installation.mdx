...
For a host-level `headroom` command on macOS or Linux, prefer `uv tool install`.
It keeps Headroom in a dedicated app environment instead of tying it to the
current project or shell Python.
