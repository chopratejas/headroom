...
class CodexWriter(ContextWriter):
    ...
    def write(
        ...
        if context_recs:
            ...
            section_content = self._build_section(context_recs)
            full_content = self._merge_into_file(agents_md, section_content)
        ...
        if memory_recs:
            ...
            section_content = self._build_section(memory_recs)
            full_content = self._merge_into_file(instructions_md, section_content)
...
class GeminiWriter(ContextWriter):
    ...
    def write(
        ...
        # Gemini has no memory file — all recommendations go to GEMINI.md
        all_recs = recommendations
        if not all_recs:
            return result

        gemini_md = project.context_file or (project.project_path / "GEMINI.md")
        section_content = self._build_section(all_recs)
        full_content = self._merge_into_file(gemini_md, section_content)
