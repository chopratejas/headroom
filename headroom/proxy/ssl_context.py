...
def build_ssl_context() -> ssl.SSLContext | None:
    """Return a custom SSLContext if any CA-bundle env var points to a file.

    Iterates ``SSL_CERT_FILE``, ``REQUESTS_CA_BUNDLE``, and
    ``NODE_EXTRA_CA_CERTS`` in that order.  The first variable that is set
    *and* points to an existing file is used to build an
    ``ssl.create_default_context()`` with the custom CA bundle loaded via
